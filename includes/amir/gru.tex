...
RNNs to capture long-term dependencies also it is simplified  LSTM cell with less parameters.\cite{web021}
\subsection{Inside GRU Cell}
\begin{figure}[H]%
    \center%
    \includegraphics[width=\textwidth]{images/amir/LSTM3-var-GRU.png}%
     % you need to add the caption for the list of figures
    \caption[GRU Cell]{a look inside GRU cell}\label{fig:gru inside}%
  \end{figure}
\indent$\bullet$\hspace{5pt}Update Gate: The update signal $z_t$
...
\indent$\bullet$\hspace{5pt}Reset Gate: The reset signal $r_t$
...
\indent$\bullet$\hspace{5pt}New memory generation: A new memory $\tilde{h}_{t}$
...
\indent$\bullet$\hspace{5pt}Hidden state: The hidden state $h_t$
